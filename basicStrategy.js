  ...
  for (let i = 0; i < chipsUsed.length - 1; i++) {
    ...
    while (currentChip.count > (nextChip.count + 1) * 2 && currentChip.original != false && nextChip.original != false) {
      if (nextChip.value % currentChip.value == 0) {
        ...
        var img = document.createElement("img");
        img.src = `$${nextChip.value}chip.png`;
        src.appendChild(img);
    ...
    while (currentChip.count < nextChip.count && currentChip.original != false && nextChip.original != false) {
      if (nextChip.value % currentChip.value == 0) {
        ...
        for (i2 = 1; i2 <= nextChip.value / currentChip.value; i2++) {
          var src = document.getElementById(`chips${currentChip.value}`);
          var img = document.createElement("img");
          img.src = `$${currentChip.value}chip.png`;
          src.appendChild(img);
        }
      }
    }
...
function double(hand) {
  ...
  if (doubleAfterSplitting == "false" && numOfSplits > 1) {
    ...
    for (i = 0; i < chipsvarD(hand).length; i++) {
      ...
      var img = document.createElement("img");
      img.src = `$${chipsvarD(hand)[i]}chip.png`;
      src.append(img);
  ...
  if (errors.length > 0) {
    for (error of errors) {
      ...
      if (error.splittable == true) {
        var paragraphContent = document.createTextNode(`you ${wrongMove} on a pair of ${error.card.toString().toLowerCase()}s vs dealers ${error.dealersCard} instead of ${correctMove}`);
      } else if (error.soft == false) {
        var paragraphContent = document.createTextNode(`you ${wrongMove} on a ${error.playersHand} vs dealers ${error.dealersCard} instead of ${correctMove}`);
      } else {
        var paragraphContent = document.createTextNode(`you ${wrongMove} on a soft ${error.playersHand} vs dealers ${error.dealersCard} instead of ${correctMove}`);
      }
